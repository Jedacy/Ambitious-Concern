  ...
  return (
    <div className={`px-5 pt-2 md:pt-4 border-${border} rounded-lg border-primary flex flex-col gap-3 flex-1 bg-${bgColor}`}>
      <div className="flex flex-row md:flex-col gap-3 items-center md:items-start">
        <img src={icon} alt="" className="w-14 h-14 md:w-20 md:h-20"/>
        <h2 className={`text-blue-950 font-Ronoto-Slab text-xl font-extrabold text-${textColor}`}>{heading}</h2>
      </div>
        
        <p className={`pb-4 md:pb-9 font-Poppins text-base text-${textColor} text-sm md:text-base`}>{paragraph}</p>
    </div>
  )
}
